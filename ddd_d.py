...
class TurtlebotObjectAligner(Node):
    ...
    def __init__(self):
        ...
        # --- 수정된 부분: patrol_forward_correction_gain 값을 0.05로 줄임 ---
        self.patrol_forward_correction_gain = 0.05 # 직진 경로 보정 게인 (튜닝 필요)
        ...
        try:
            ...
            if self.current_state == self.STATE_ALIGNING:
                ...
                if object_roi is not None:
                    x, y, w, h = object_roi
                    # 빨간색 물체 표시: BGR (0, 0, 255)
                    cv2.rectangle(processed_image, (x, y), (x + w, y + h), (0, 0, 255), 2)
                    cv2.putText(processed_image, "Red Object", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
                ...
                if distance_remaining_in_segment <= 0.01: # 한 변의 이동이 거의 완료되면
                    ...
                    if self.segment_start_pose:
                        ...
                        if abs(yaw_error_for_segment) < self.patrol_yaw_tolerance:
                            ...
                            # --- 추가된 디버그 로그 ---
                            self.get_logger().info(f"{YELLOW}DEBUG: Yaw Error: {math.degrees(yaw_error_for_segment):.2f} deg, Pre-clip Angular Z: {target_angular_z:.2f} rad/s{RESET}")
    ...
    def detect_and_get_object_info(self, image):
        ...
        # 빨간색의 첫 번째 범위 (Hue: 0 ~ 10)
        lower_red1 = np.array([0, 100, 100])
        upper_red1 = np.array([10, 255, 255])
        mask1 = cv2.inRange(hsv, lower_red1, upper_red1)

        # 빨간색의 두 번째 범위 (Hue: 160 ~ 179)
        lower_red2 = np.array([160, 100, 100])
        upper_red2 = np.array([179, 255, 255])
        mask2 = cv2.inRange(hsv, lower_red2, upper_red2)

        # 두 마스크를 결합하여 전체 빨간색 영역 감지
    ...
    def linear_return_timer_callback(self):
        ...
        if distance_to_go_back <= 0.05: # 5cm 이내면 복귀 완료 (허용 오차)
            ...
            # 3.0을 곱하는 것은 각도 보정 게인을 후진 시 더 강하게 적용하기 위함 (필요 시 조절)
            twist_msg.angular.z = self.align_kp_angular * angle_error * 3.0
            max_angular_vel = 0.2 # 후진 중 최대 회전 속도 제한
            twist_msg.angular.z = np.clip(twist_msg.angular.z, -max_angular_vel, max_angular_vel)

            # --- 이 부분은 너무 자주 출력될 수 있어 주석 처리했습니다. ---
            # if not self._linear_return_started_log:
            #     self.get_logger().info(f"{MAGENTA}↩️ 후진 중... 남은 거리: {distance_to_go_back:.2f}m. 보정 각도: {math.degrees(twist_msg.angular.z):.2f}도{RESET}")
            #     self._linear_return_started_log = True
            # else:
            #     self.get_logger().debug(f"↩️ 후진 중... 남은 거리: {distance_to_go_back:.2f}m. 보정 각도: {math.degrees(twist_msg.angular.z):.2f}도")
    ...
    def complete_return_process(self):
        """복귀 프로세스 완료 후 순찰 모드로 돌아갑니다."""
        self.log_once(BLUE, "🔄 복귀 프로세스 완료! 순찰 모드 재개합니다.{RESET}")
        self.stop_robot() # 최종 정지

        # _object_handled 플래그를 True로 설정하여, 다시 순찰 중에는 객체를 무시하도록 함
        self._object_handled = True
        self._object_handled_msg_logged = False # "더 이상 인식 안함" 메시지 초기화를 위해

        self.reset_alignment_flags() # 정렬 관련 플래그 초기화 (다음 객체 감지를 위해)
        ...
        self.segment_start_pose = self.saved_patrol_state['segment_start_pose']
        ...
        self.current_state = self.STATE_PATROLLING # 순찰 모드로 전환

        self.get_logger().info(f"{GREEN}🟢 순찰 재개 준비 완료. 현재 상태: {self.patrol_motion_state}, 코너 인덱스: {self.current_patrol_idx}{RESET}")


    def destroy_node(self):
        """노드 종료 시 깔끔하게 자원 정리"""
        self.get_logger().info("노드 종료 중. 자원 정리...")
        self.stop_robot() # 로봇 정지
        cv2.destroyAllWindows() # OpenCV 창 닫기 (필요 시 깔끔하게)
...
def main(args=None):
    ...
    print("  - 순찰 중 **빨간색 물체**가 감지되면, 로봇은 순찰을 중단하고 물체에 정렬하여 가까이 접근한 후 정지합니다.")
    ...
    print("  - 복귀가 완료되면, 로봇은 다시 순찰을 재개합니다. (단, 한 번 처리된 객체는 더 이상 인식하지 않습니다.)")
    print("--- 초기화 중 ---")
